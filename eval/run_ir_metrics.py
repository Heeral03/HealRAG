...
IR_EVAL_DATASET = [
    ...
    },
    {
        ...
        "relevant_docs": ["doc_024_ips_diagnostic_results.txt", "doc_025_ips_procedures_history.txt", "doc_026_ips_diagnostic_results.txt", "doc_027_ips_procedures_history.txt"]
    },
    {
    ...
    },
    {
        ...
        "relevant_docs": ["doc_112_nhs_dspt_compliance.txt"]
    },
    {
        "query": "How does the NHS National Data Opt-out apply to secondary research?",
        "relevant_docs": ["doc_113_nhs_national_data_opt_out.txt"]
    },
    {
        "query": "What are the required FHIR UK Core profiles for patient demographics?",
        "relevant_docs": ["doc_114_uk_core_fhir_patient_profile.txt"]
    }
]

def calculate_ir_metrics(k_list=[1, 3, 5, 10]):
    print("=======================================================================")
    print("      STANDARD INFORMATION RETRIEVAL (IR) BENCHMARK EVALUATION        ")
    print("      (Evaluating FAISS + sentence-transformers/all-MiniLM-L6-v2)    ")
    print("=======================================================================")
    ...
    for q_idx, item in enumerate(IR_EVAL_DATASET):
        query = item["query"]
        ...
        print(f"\n[{q_idx+1:02d}/{total_queries}] Query: '{query[:55]}...'")
        print(f"     Relevant Docs ({len(relevant_docs)}): {list(relevant_docs)}")
    ...
    qps = round(1000 / avg_latency_ms, 2)

    print("\n-----------------------------------------------------------------------")
    print(" 1. STANDARD RETRIEVAL PERFORMANCE MATRIX")
    print("-----------------------------------------------------------------------")
    summary = {}
    for k in k_list:
        ...
        print(f" - k={k:<2d} | Hit Rate: {hit_rate:>6.2f}% | Mean Recall: {mean_recall:>6.2f}% | MRR: {mrr:.4f}")

    print("\n-----------------------------------------------------------------------")
    print(" 2. RETRIEVAL ENGINE SCALING & THROUGHPUT STATS")
    print("-----------------------------------------------------------------------")
    print(f" - Total Benchmark Queries : {total_queries}")
    print(f" - Vector DB Corpus Size   : {len(retriever.metadata)} Chunks (384-dim Dense Embeddings)")
    print(f" - Total Batch Runtime     : {total_time_ms} ms")
    print(f" - Avg Latency per Query   : {avg_latency_ms} ms")
    print(f" - Retrieval Throughput    : {qps} Queries Per Second (QPS)")

    output_payload = {
        "benchmark_metadata": {
            "query_count": total_queries,
            "embedding_model": "sentence-transformers/all-MiniLM-L6-v2",
            "vector_index_type": "FAISS IndexFlatIP (Cosine Similarity)",
